...
- HIGH: none.
- MEDIUM: `scripts/publish_pr.py` can commit, push, and create/update PRs when policy gates pass. It now persists runtime state after irreversible actions, resumes partial failures only while current verdict permits publication, and blocks unsafe target branches/base branches.
