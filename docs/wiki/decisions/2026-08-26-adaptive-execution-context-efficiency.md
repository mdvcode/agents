...
The task budget controller tracks model calls, uncached input, output, elapsed time, repairs, and escalations. Model-call and token ceilings are soft cost signals: exceeding them selects economy execution for mandatory work and sheds optional roles without requesting approval. Elapsed-time, repair-attempt, and escalation ceilings remain hard bounds that may require approval. Mandatory verification and security gates are never skipped.
