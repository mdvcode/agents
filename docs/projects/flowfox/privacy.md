...
- Completed non-HIGH issue work may be committed, pushed, and opened or updated as a PR without a separate user approval when `.agent-policy.yaml` allows it, using the repository's configured git identity and the authenticated GitHub account. Automated publication does not allow publishing private memory, raw sensitive screenshots, unreviewed issue notes, agent or control-plane files, or text that mentions agents, Codex, AI assistance, or automation.
