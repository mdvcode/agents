...
- Target project branch names are public PR metadata. Each project may configure any non-empty set of safe Git prefixes in `.agent-policy.yaml` or `.agent-repositories.yaml`, such as `feat/`, `fix/`, `chore/`, `docs/`, `release/`, or a team prefix. Do not include agents, Codex, AI, automation, private control-plane paths, or internal tooling names.
