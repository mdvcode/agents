...
class RuntimeConfigurationError(ValueError):
    """Raised when runtime configuration violates the Step 2 single-provider contract."""
...
def load_runtime_config(path: Path = RUNTIME_CONFIG) -> dict[str, Any]:
    ...
    if runtime.get("transport") != "local_subscription":
        raise RuntimeConfigurationError("Step 2 codex-cli runtime must use local_subscription transport")
    if runtime.get("api_required") is not False:
        raise RuntimeConfigurationError("Step 2 codex-cli runtime must not require a provider API")
    if runtime.get("model_router") is not False:
        raise RuntimeConfigurationError("Step 2 runtime configuration must disable model_router")
    return runtime
...
def create_runtime(
    *,
    ...
    if command_override and selected == "codex-cli" and not test_mode:
        raise RuntimeConfigurationError(
            "production codex-cli command overrides are restricted to harness test mode"
        )
    if selected != "codex-cli":
        raise RuntimeConfigurationError(f"unsupported runtime provider: {selected!r}")
    executor_command = command_override if test_mode and command_override else str(config.get("executor_command", ""))
    if not executor_command:
        raise RuntimeConfigurationError("codex-cli runtime executor_command is required")
