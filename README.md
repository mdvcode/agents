...
По умолчанию `agent task` выбирает режим автоматически: обычная локальная правка идет по быстрому маршруту с двумя модельными ролями (реализация и ревью), а рискованные или широкие задачи — по полному. Быстрый маршрут ограничен 15 минутами и автоматически переключается на полный, если изменение затронуло больше пяти файлов, превысило 200 строк или коснулось защищенных областей. Режим можно задать явно: `agent task --mode fast "..."` или `agent task --mode full "..."`.
