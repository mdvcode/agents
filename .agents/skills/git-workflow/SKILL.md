...
## Flowfox Policy Publishing
- Read `.agent-policy.yaml` before deciding whether Flowfox work can be committed, pushed, or opened as a PR.
- Before publishing Flowfox UI or user-visible work, capture local-site evidence: screenshots for static states and video or trace evidence for interaction-heavy flows.
- For completed LOW/MEDIUM Flowfox issue work, publish without a separate user approval when checks pass, no protected paths are touched, no blockers remain, and required local evidence is provided.
